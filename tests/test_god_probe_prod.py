...
def test_prod_god_awarded(dev_client: TestClient) -> None:
    ...
    for idx, domain in enumerate((payload.get("god_probe") or {}).keys()):
        meta = payload["god_probe"][domain]
        tier = tiers[idx]
        ...
        if first_tier in {"SS", "GOD"}:
            ...
            assert meta.get("probe_reason") == "probe_blocked_below_ss", domain
...
def test_prod_god_denied_high_rubric(dev_client: TestClient) -> None:
    ...
    for domain, meta in (payload.get("god_probe") or {}).items():
        ...
        if first_tier in {"SS", "GOD"}:
            ...
            assert "blocked_progress" in (meta.get("reasons") or [])
        else:
            # below SS never attempted probe two
            assert meta.get("probe_reason") in (None, "probe_blocked_below_ss")


def test_prod_long_fail_fast_blocks_probes(dev_client: TestClient) -> None:
    payload = _run(
        dev_client,
    ...
    for domain, meta in (payload.get("god_probe") or {}).items():
        assert meta.get("probe2_served") is False, domain
        assert meta.get("probe_reason") == "blocked_fail_fast"
        reasons = meta.get("reasons") or []
        assert "blocked_fail_fast" in reasons
