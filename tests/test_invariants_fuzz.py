...
def test_dev_run_seed_invariants(tmp_path_factory, invariants_tracker, seed):
    ...
    if not (
        long_pass.get("steps") == 160
        and long_pass.get("open_used") == 8
        and not long_pass.get("stop_reason")
    ):
        invariants_tracker["long_pass_cap"].append(
            (
                seed,
                long_pass.get("steps"),
    ...
    if not (
        long_fail.get("steps", 999) < 160
        and long_fail.get("stop_reason") == "fail_fast_long"
    ):
        invariants_tracker["long_fail_early"].append(
            (
                seed,
                long_fail.get("steps"),
